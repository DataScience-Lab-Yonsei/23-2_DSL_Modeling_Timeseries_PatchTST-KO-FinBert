...
class PatchTST(BaseWindows):
    ...
    **Parameters:**
    ...
    `patch_len`: int=32, length of patch.
...
# %%
reader = pd.read_csv('/content/drive/MyDrive/유튜브스크립트/부읽남_최종.csv', parse_dates = ['ds'], index_col = 0).sort_values('ds').reset_index(drop = True)
han = pd.read_csv('/content/drive/MyDrive/유튜브스크립트/한경글로벌마켓_최종.csv', parse_dates = ['ds'], index_col = 0).sort_values('ds').reset_index(drop = True)
pro3= pd.read_csv('/content/drive/MyDrive/유튜브스크립트/3pro_sent.csv', parse_dates = ['ds'], index_col = 0).sort_values('ds').reset_index(drop = True)
talent = pd.read_csv('/content/drive/MyDrive/유튜브스크립트/talent_sent.csv', parse_dates = ['ds'], index_col = 0).sort_values('ds').reset_index(drop = True)
data_list = [reader, han, pro3]
for data in data_list:
  try:
    data['script_linear'] = data['script'].interpolate(method='linear')
    data['script'] = data['script_linear'].rolling(window = 5).mean()
    data['script'].iloc[:4] = data['script_linear'].iloc[:4]
  except:
    pass

  data['title_linear'] = data['title'].interpolate(method='linear')
  data['title'] = data['title_linear'].rolling(window = 5).mean()
  data['title'].iloc[:4] = data['title_linear'].iloc[:4]


# %%

# 부읽남
reader_df = reader.copy()
reader_df.columns = ['unique_id', 'ds', 'y', 'title_y', 'd', 'e']

reader_df_y = reader_df[['unique_id', 'ds', 'y']]

reader_df_title = reader_df[['unique_id', 'ds', 'title_y']]
reader_df_title.columns = ['unique_id', 'ds', 'y']
reader_df_title['unique_id'] = '부읽남_제목'

# 한경
han_df = han.copy()
han_df.columns = ['unique_id', 'ds', 'y', 'title_y', 'd', 'e']
han_df_y = han_df[['unique_id', 'ds', 'y']]
han_df_y = han_df_y.groupby(['ds', 'unique_id']).mean().reset_index()

han_df_title = han_df[['unique_id', 'ds', 'title_y']]
han_df_title.columns = ['unique_id', 'ds', 'y']
han_df_title['unique_id'] = '한경_제목'

# 삼프로 15일
pro3_df = pro3.copy()
pro3_df.columns = ['unique_id', 'ds', 'y', 'w']
pro3_df = pro3_df[['unique_id', 'ds', 'y']]

# 달란트
talent_df = talent.copy()
talent_df['y_ma'].iloc[:4] = talent_df['y'].iloc[:4]
talent_df = talent_df[['unique_id', 'ds', 'y_ma']]
talent_df.columns = ['unique_id', 'ds', 'y']


# 시차 상관계수 반영
reader_df['y'] = reader_df['y'].shift(2)
reader_df = reader_df.iloc[2:,:]
pro3_df['y'] = pro3_df['y'].shift(15)
pro3_df = pro3_df.iloc[15:,:]



reader_df_y = reader_df_y.groupby(['ds', 'unique_id']).mean().reset_index()
reader_df_title = reader_df_title.groupby(['ds', 'unique_id']).mean().reset_index()
han_df_title = han_df_title.groupby(['ds', 'unique_id']).mean().reset_index()
pro3_df = pro3_df.groupby(['ds', 'unique_id']).mean().reset_index()
...
# %%
# # 부읽남만 사용
concat_df = pd.concat([df.iloc[:, :3], reader_df_y, reader_df_title]).reset_index(drop = True)
train_df = concat_df.loc[concat_df['ds'] < test_time]
test_df = concat_df.loc[concat_df['ds'] >= test_time]

static_df = pd.DataFrame({'unique_id' : ['kosdaq', f'{word1}', f'{word2}'],
                          'kosdaq' : [1, 1, 1],
                          f'{word1}' : [1, 1,1 ],
                          f'{word2}' : [1, 1, 1]})
...
# %%
# 한경만 사용
concat_df = pd.concat([df.iloc[:, :3], han_df_y, han_df_title]).reset_index(drop = True)
train_df = concat_df.loc[concat_df['ds'] < test_time]
test_df = concat_df.loc[concat_df['ds'] >= test_time]

static_df = pd.DataFrame({'unique_id' : ['kosdaq', f'{word3}', f'{word4}'],
                          'kosdaq' : [1, 1, 1],
                          f'{word3}' : [1, 1,1 ],
                          f'{word4}' : [1, 1, 1]})
...
# 한경제목만 사용용
concat_df = pd.concat([df.iloc[:, :3], han_df_y]).reset_index(drop = True)
train_df = concat_df.loc[concat_df['ds'] < test_time]
test_df = concat_df.loc[concat_df['ds'] >= test_time]

static_df = pd.DataFrame({'unique_id' : ['kosdaq', f'{word3}'],
                          'kosdaq' : [1, 1],
                          f'{word3}' : [1, 1],
                          })
...
# %%
# 삼프로만 사용
concat_df = pd.concat([df.iloc[:, :3], pro3_df]).reset_index(drop = True)
train_df = concat_df.loc[concat_df['ds'] < test_time]
test_df = concat_df.loc[concat_df['ds'] >= test_time]

static_df = pd.DataFrame({'unique_id' : ['kosdaq', f'{word5}'],
                          'kosdaq' : [1, 1],
                          f'{word3}' : [1, 1],
                          })
...
# %%
# 달란트tv만 사용
concat_df = pd.concat([df.iloc[:, :3], talent_df]).reset_index(drop = True)
train_df = concat_df.loc[concat_df['ds'] < test_time]
test_df = concat_df.loc[concat_df['ds'] >= test_time]

static_df = pd.DataFrame({'unique_id' : ['kosdaq', f'{word6}'],
                          'kosdaq' : [1, 1],
                          f'{word6}' : [1, 1]})
...
# 부읽남 미사용
concat_df = pd.concat([df.iloc[:,:3], han_df_y, han_df_title, pro3_df, talent_df]).reset_index(drop = True)
train_df = concat_df.loc[concat_df['ds'] < test_time]
test_df = concat_df.loc[concat_df['ds'] >= test_time]

static_df = pd.DataFrame({'unique_id' : ['kosdaq', f'{word3}', f'{word4}', f'{word5}', f'{word6}'],
                          'kosdaq' : [1, 1, 1, 1, 1],
                          f'{word3}' :[1, 0, 1, 1, 1],
                          f'{word4}' : [1, 1, 0, 1, 1],
                          f'{word5}' : [1, 1, 1, 1, 1],
